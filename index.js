...
app.post('/payment/initialize', async (req, res) => {
    try {
        ...
        const response = await axios.post(`${API_BASE_URL}/transaction/initialize`, { email, amount: Math.round(amount * 100), callback_url: `http://localhost:3000/payment/callback` }, { headers: { Authorization: `Bearer ${ERCASPAY_SECRET_KEY}` } });
        res.status(200).json(response.data);
    } catch (error) { res.status(500).json({ message: 'Failed to initialize payment.' }); }
});
...
// THIS IS THE DEMO MODE WITHDRAWAL LOGIC
